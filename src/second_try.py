...
class Agent:
	...
	def get_model(self, input_shape, nb_actions):
		model = Sequential()
		model.add(Conv2D(16, (3, 3), input_shape = (1,) + input_shape, activation = 'relu',\
		...
		model.add(Flatten())
		model.add(Dense(input_dim = model.layers[-1].output_shape, units = 24, activation = 'relu'))
		model.add(Dense(input_dim = model.layers[-1].output_shape, units = nb_actions, activation = 'softmax'))
	...
	def get_action(self, current_state):
		...
		temp_current_state = np.expand_dims(temp_current_state, axis = 0)
		if (np.random.rand() <= self.epsilon):
			return random.randrange(self.nb_actions)
		best_action = np.argmax(self.model.predict(np.array(temp_current_state))[0])
		return best_action
	...
	def exp_replay(self, batch_size):
		...
		for (current_state, next_state, reward, action, has_completed) in batch:
			...
			target = reward
			if (has_completed):
				target = reward + self.discount_factor * np.max(self.model.predict(np.array(temp_current_state))[0])
			target_f = self.model.predict(np.array(temp_current_state))
...
if (__name__ == '__main__'):
	...
	try:
		agent.load_weights('second_attempt.h5')
	except:
		pass
	env.close()
	num_episodes = 2000
	for e in xrange(1, num_episodes + 1):
		...
		env.close()
		if (e % 5 == 0):
			agent.save_weights('second_attempt.h5')